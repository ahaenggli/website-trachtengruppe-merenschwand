...
'vorstand.html|Vorstand'  =>array('praesidenten.html|List aller Pr&auml;sidentinnen'=>''),
...
$array = explode('|', $array);
$array = $array[0];
 echo (empty($array) || in_array($in_array, $array))? '':'<url><loc>http://www.trachtengruppe-merenschwand.ch/'.$array."</loc><lastmod>2015-05-01</lastmod></url>\n";
$in_array[] = $array;
}else{
foreach($array as $men=>$it){
$men = explode('|', $men);
$men = $men[0];
echo (empty($men) || in_array($men, $in_array))? "":'<url><loc>http://www.trachtengruppe-merenschwand.ch/'.$men."</loc><lastmod>2015-05-01</lastmod></url>\n";
$in_array[] = $men;
if(is_array($it)) xml($it);
}
     }
}
